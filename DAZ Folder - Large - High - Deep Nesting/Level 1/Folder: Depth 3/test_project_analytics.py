...
from pages.login import accept_cookies
...
class TestAnalyticsPage:
    def test_private_project(self, driver, default_project, must_be_logged_in):
        ...
        assert (
            'Analytics are not available for private projects.'
            in analytics_page.private_project_message.text
        )
...
class TestNodeAnalytics:
    def test_unique_visits_graph(self, session, driver, public_project_node):
        ...
        chart_container = driver.find_element(
            By.XPATH, "//osf-line-chart[h2[normalize-space()='Unique visits']]"
        )
        canvas_elements = chart_container.find_elements(By.TAG_NAME, 'canvas')

        assert len(canvas_elements) > 0, 'canvas is not present in osf-line-chart'
    ...
    def test_tod_visits_graph(self, session, driver, public_project_node):
        ...
        chart_container = driver.find_element(
            By.XPATH, "//osf-bar-chart[h2[normalize-space()='Time of day of visits']]"
        )
        canvas_elements = chart_container.find_elements(By.TAG_NAME, 'canvas')

        assert len(canvas_elements) > 0, 'canvas is not present in osf-bar-chart'
    ...
    def test_popular_pages_graph(self, session, driver, public_project_node):
        ...
        chart_container = driver.find_element(
            By.XPATH, "//osf-bar-chart[h2[normalize-space()='Popular pages']]"
        )
        canvas_elements = chart_container.find_elements(By.TAG_NAME, 'canvas')
